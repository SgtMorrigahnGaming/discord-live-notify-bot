...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .rr-add-mapping { display: flex; gap: 6px; margin-top: 4px; }
...
<script>
  ...
  function renderReactionRoles(panels, channels, roles) {
    state.rrData = { panels, channels, roles }; // cached so Edit/Done can toggle instantly, no refetch
    ...
    el.innerHTML = panels.map(p => {
      ...
      const mappingLine = m => `${m.emoji_id ? `[custom:${m.emoji_name}]` : m.emoji_name} → @${roleName(roles, m.role_id)}`;

      const summary = `
        <div class="rr-summary">
          ${p.mappings.map(m => `<div class="rr-summary-row">${mappingLine(m)}</div>`).join('')
          ...
          <div class="rr-edit-section">
            ...
            ${p.mappings.map(m => `
              <div class="rr-mapping-edit-row" data-emoji-id="${m.emoji_id || ''}" data-emoji-name="${m.emoji_name}">
                <span class="rr-mapping-emoji">${m.emoji_id ? `[custom:${m.emoji_name}]` : m.emoji_name}</span>
            ...
            <div class="rr-add-mapping">
              <input type="text" placeholder="New emoji" class="rr-emoji-input" />
    ...
    el.querySelectorAll('[data-action="toggle-edit"]').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        renderReactionRoles(state.rrData.panels, state.rrData.channels, state.rrData.roles);
      });
    });
